...
// TODO(route): this page lives at /users/[userId] for now. Once Keycloak login
// lands it should move to /dashboard and derive the user from the access token's
// `sub` claim — a user may only view their own dashboard — dropping the [userId]
// route param and the view-anyone-by-id access it currently implies.
...
export default function UserDashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getUserDashboard(userId, controller.signal);
        setState({ kind: "ready", data });
      } catch (err) {
        if (controller.signal.aborted) return;
        // 404 (no such user) and 5xx (aggregator/downstream failure) are
        // distinct, actionable cases — not one generic "something went wrong".
