<#-- EMS login shell. This is the stock Keycloak `login.ftl` (26.0) with ONE
     addition: a branded subtitle under the headline. The form itself — action,
     hidden CSRF/credentialId inputs, `username`/`password` field names, labels,
     per-field validation, password-visibility toggle, remember-me, forgot-
     password and registration links — is taken verbatim from the base template
     so no flow logic is hand-rolled. We do NOT override template.ftl. -->
<#import "template.ftl" as layout>
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    <#if section = "header">
        ${msg("loginAccountTitle")}
    <#elseif section = "form">
        <p class="ems-subtitle">${msg("emsLoginSubtitle")}</p>
        <div id="kc-form">
